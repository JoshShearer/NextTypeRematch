...
describe("Comps_Navigation_Header", () => {
  ...
  // it("should render a '' on the Comps_Navigation_Header and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_Navigation_Header.rename(oneProduct);
  //   renderWithRematchStore(<Comps_Navigation_Header />, store);
  ...
  // it("should reset the Comps_Navigation_Header to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_Navigation_Header.rename(otherProduct);
  //   renderWithRematchStore(<Comps_Navigation_Header />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
